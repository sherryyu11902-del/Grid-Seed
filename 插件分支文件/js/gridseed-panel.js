...
(function () {
  ...
  function initStarterKitBlock() {
    ...
    if (btn && !btn.dataset.bound) {
      ...
      btn.addEventListener("click", function () {
        ...
        try {
          fetch("http://127.0.0.1:7593/ingest/c5bfbc7e-0c13-4abf-baaa-00396ebb7eef", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              "X-Debug-Session-Id": "5d122b",
            },
            body: JSON.stringify({
              sessionId: "5d122b",
              runId: "starterkit_click",
              hypothesisId: "H5",
              location: "gridseed-panel.js:btnStarterCreate",
              message: "starterkit_click_start",
              data: { templateId: selectedPaperVariantId, theme: theme, lg: lg },
              timestamp: Date.now(),
            }),
          }).catch(function () {});
        } catch (eDbg) {}
          ...
          function (r1) {
            ...
            try {
              fetch("http://127.0.0.1:7593/ingest/c5bfbc7e-0c13-4abf-baaa-00396ebb7eef", {
                ...
                body: JSON.stringify({
                  ...
                  hypothesisId: "H4",
                  location: "gridseed-panel.js:btnStarterCreate",
                  message: "starterkit_dispatch_result",
                  data: { r1: s1 },
                  timestamp: Date.now(),
                }),
              }).catch(function () {});
            ...
            if (typeof window.gridseedBuildLayoutPlanPayload === "function") {
              ...
              if (rest) {
                ...
                // #region agent debug log
                try {
                  var _restParts = rest.split("|");
                  var _preview = _restParts.slice(0, 10).join("|");
                  fetch("http://127.0.0.1:7593/ingest/c5bfbc7e-0c13-4abf-baaa-00396ebb7eef", {
                    method: "POST",
                    headers: {
                      "Content-Type": "application/json",
                    ...
                    body: JSON.stringify({
                      ...
                      hypothesisId: "H1",
                      location: "gridseed-panel.js:btnStarterCreate",
                      message: "layoutplan_payload_built",
                      data: { restPreview: _preview, restPartsLen: _restParts.length },
                      timestamp: Date.now(),
                    }),
                  }).catch(function () {});
                } catch (eDbg3) {}
                  ...
                  try {
                    fetch("http://127.0.0.1:7593/ingest/c5bfbc7e-0c13-4abf-baaa-00396ebb7eef", {
                      ...
                      body: JSON.stringify({
                        ...
                        hypothesisId: "H3",
                        location: "gridseed-panel.js:btnStarterCreate",
                        message: "layoutplan_dispatch_result",
                        data: { r2: s2 },
                        timestamp: Date.now(),
                      }),
                    }).catch(function () {});
  ...
  function initGridComposer() {
    ...
    function debugLayoutProbe(tag) {
      try {
        var rootEl = document.getElementById("panel-templates");
        var mainEl = document.querySelector(".main.main--scroll");
        var compEl = document.getElementById("gridComposerRoot");
        var layoutEl = document.getElementById("gcLayoutGrid");
        var actionEl = document.querySelector(".gc-preview-actions");
        var over = [];
        [rootEl, mainEl, compEl, layoutEl, actionEl].forEach(function (el) {
          if (!el) return;
        ...
        fetch("http://127.0.0.1:7593/ingest/c5bfbc7e-0c13-4abf-baaa-00396ebb7eef", {
          method: "POST",
          headers: { "Content-Type": "application/json", "X-Debug-Session-Id": "5d122b" },
          body: JSON.stringify({
            sessionId: "5d122b",
            runId: "panel_overflow_probe",
            hypothesisId: "H1",
            location: "gridseed-panel.js:initGridComposer",
            message: "layout_probe",
            data: {
              tag: tag,
              panelClient: rootEl ? rootEl.clientWidth : -1,
              panelScroll: rootEl ? rootEl.scrollWidth : -1,
              mainClient: mainEl ? mainEl.clientWidth : -1,
              mainScroll: mainEl ? mainEl.scrollWidth : -1,
              composerClient: compEl ? compEl.clientWidth : -1,
              composerScroll: compEl ? compEl.scrollWidth : -1,
              layoutClient: layoutEl ? layoutEl.clientWidth : -1,
              layoutScroll: layoutEl ? layoutEl.scrollWidth : -1,
              actionsClient: actionEl ? actionEl.clientWidth : -1,
              actionsScroll: actionEl ? actionEl.scrollWidth : -1,
    ...
    function debugPreviewPlacement(tag) {
      try {
        var rootEl = document.getElementById("gridComposerRoot");
        var headerEl = rootEl ? rootEl.querySelector(".gc-header") : null;
        var starterAcc = rootEl ? rootEl.querySelector(".gc-acc") : null;
        var previewAcc = document.getElementById("gcPreviewWrap");
        var allAcc = rootEl ? rootEl.querySelectorAll(".gc-acc") : [];
        var order = [];
        var i;
        for (i = 0; i < allAcc.length; i++) {
          var s = allAcc[i].querySelector("summary");
          order.push(s ? String(s.textContent || "").replace(/\s+/g, " ").trim() : "no-summary");
        }
        var hr = headerEl ? headerEl.getBoundingClientRect() : null;
        var sr = starterAcc ? starterAcc.getBoundingClientRect() : null;
        var pr = previewAcc ? previewAcc.getBoundingClientRect() : null;
        // #region agent log
        fetch("http://127.0.0.1:7593/ingest/c5bfbc7e-0c13-4abf-baaa-00396ebb7eef", {
          method: "POST",
          headers: { "Content-Type": "application/json", "X-Debug-Session-Id": "5d122b" },
          body: JSON.stringify({
            sessionId: "5d122b",
            runId: "preview_top_probe",
            hypothesisId: "H2",
            location: "gridseed-panel.js:initGridComposer",
            message: "preview_placement_probe",
            data: {
              tag: tag,
              previewExists: !!previewAcc,
              previewHidden: previewAcc ? !!previewAcc.hidden : null,
              accordionOrder: order,
              headerTop: hr ? hr.top : null,
              starterTop: sr ? sr.top : null,
              previewTop: pr ? pr.top : null,
              previewBeforeStarter:
                !!(previewAcc && starterAcc && previewAcc.compareDocumentPosition(starterAcc) & Node.DOCUMENT_POSITION_FOLLOWING),
            },
            timestamp: Date.now(),
          }),
        }).catch(function () {});
    ...
    function renderLayouts() {
      ...
      debugLayoutProbe("render_layouts");
    }
    ...
    debugLayoutProbe("init_done");
    debugPreviewPlacement("init_done");
    window.addEventListener("resize", function () {
      debugLayoutProbe("window_resize");
      debugPreviewPlacement("window_resize");
    });
    ...
    if (btnCreate) {
      btnCreate.addEventListener("click", function () {
        ...
        debugLayoutProbe("after_create_click");
        debugPreviewPlacement("after_create_click");
      });
    }
