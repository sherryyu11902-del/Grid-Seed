...
## 📌 Scope of this repo · 仓库能呈现什么 / 不能呈现什么

| **English** | **中文** |
|-------------|----------|
| **Illustrator plugin reality** — GridSeed is a **CEP panel inside Adobe Illustrator**. HTML on GitHub Pages (e.g. the keynote) can explain the product and show static slides, but **cannot replace the full in-app workflow** (live artboard, ExtendScript actions, real panel behavior). | **插件与网页的分工** — GridSeed 是 **运行在 Illustrator 内部的 CEP 面板**。GitHub Pages 上的 HTML（例如 keynote）适合叙事与静态展示，**无法代替在 Illustrator 里的完整使用流程**（实时画板、JSX 执行、真实面板交互）。 |
| **What to use for “full” experience** — Install the extension from the **complete folder** (see **Google Drive** below), watch the **YouTube** walkthrough, and use Illustrator locally. | **完整体验方式** — 请从 **Google Drive 完整文件夹** 取得工程并按安装流程置入 Illustrator，配合 **YouTube 视频**，在本机 Illustrator 中实际操作。 |

---

## 📦 Full package on Google Drive · 网盘完整下载（含大文件）

| **English** | **中文** |
|-------------|----------|
| **Why not only GitHub?** — GitHub enforces **per-file limits** (roughly **100 MB hard block**, **~50 MB warning**). Courses or teams often ask for smaller pushes (e.g. **~25 MB**). Large **video (.MOV)**, **PDF**, or a **full project archive** may not fit repository rules—host them externally. | **为何需要网盘？** — GitHub 对**单文件体积**有限制（约 **100 MB** 无法推送，**约 50 MB** 会收到警告）。部分课程还要求仓库更小（例如 **约 25 MB** 内）。**高清视频、大体量 PDF、完整压缩包** 等更适合放在网盘，而非只依赖 Git。 |
